/**
 * Fireblocks API
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: 1.6.2
 * Contact: support@fireblocks.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */
/**
 *
 * @export
 * @interface Transaction
 */
export interface Transaction {
    /**
     *
     * @type {string}
     * @memberof Transaction
     */
    'id': string;
    /**
     *
     * @type {string}
     * @memberof Transaction
     */
    'state': TransactionStateEnum;
    /**
     *
     * @type {number}
     * @memberof Transaction
     */
    'timestamp'?: number;
    /**
     *
     * @type {string}
     * @memberof Transaction
     */
    'instructionId'?: string;
}
export declare const TransactionStateEnum: {
    readonly Submitted: "SUBMITTED";
    readonly Queued: "QUEUED";
    readonly PendingAuthorization: "PENDING_AUTHORIZATION";
    readonly PendingSignature: "PENDING_SIGNATURE";
    readonly Broadcasting: "BROADCASTING";
    readonly Pending3RdPartyManualApproval: "PENDING_3RD_PARTY_MANUAL_APPROVAL";
    readonly Pending3RdParty: "PENDING_3RD_PARTY";
    readonly Pending: "PENDING";
    readonly Confirming: "CONFIRMING";
    readonly Confirmed: "CONFIRMED";
    readonly Completed: "COMPLETED";
    readonly PartiallyCompleted: "PARTIALLY_COMPLETED";
    readonly PendingAmlScreening: "PENDING_AML_SCREENING";
    readonly Cancelling: "CANCELLING";
    readonly Cancelled: "CANCELLED";
    readonly Rejected: "REJECTED";
    readonly Blocked: "BLOCKED";
    readonly Failed: "FAILED";
    readonly Timeout: "TIMEOUT";
};
export type TransactionStateEnum = typeof TransactionStateEnum[keyof typeof TransactionStateEnum];
